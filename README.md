## Предисловие
Во всех проектных работах так или иначе присутсвтует необходимость ограничить набор доступных действий для некоторых пользователей, равно как и возможность регистрации и авторизации. Выполнив данное задание, вы получите библиотеку, которую сможете использовать в своём проекте.

## Задание
Ваша задача заключается в реализации системы авторизации и контроля за действиями пользователей.

Создайте где-нибудь файл `auth-lib.js`, который будет являться результатом выполнения данного задания. Как будете готовы - создайте новый проект в нашем [gitlab](https://git.etagi.com/) и залейте туда `auth-lib.js`.

Есть два основных фактора, которые повлияют на **оценку вашей работы**:

1. Количество успешно пройденных автоматических тестов
2. Качество кода
...
**Почему это важно?**
Потому что я не знаю и не хочу знать заранее как именно вы реализуете эти функции, какие структуры при этом будете использовать. Оценка вашей работы будет частично производиться автоматическими инструментами, которые будут опираться на спецификацию выше.
...
Приветствуется реализация вспомогательных функций, которые бы сделали работу с пользователями, группами и правами нагляднее.
...
var secureIncreaseCounter = withWrapper(increaseCounter, "canIncreaseCounter");
...
- Добавить в эту директорию (Week_02/JS/practice-assignment, начиная с корня проекта) вашу реализацию (файл auth-lib.js)
...
Выше я показал, как можно представить права, пользователей и группы в JavaScript, где права и группы имеют строковое представление. 
Такое представление не является самым удачным в контексте данного задания) Но вас никто не заставляет его использовать)
Если вы всё же очень хотите, чтобы права и группы имели строковое представление, можете генерировать случайные строки.
...
Первый аргумент функции `apply`- объект, который станет значением `this` в теле вызванной функции (об этом мы поговорим подробнее на второй лекции, сейчас можно в качестве первого аргумента указать `null`)
